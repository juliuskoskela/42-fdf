...
/*   Updated: 2021/01/13 01:00:14 by jkoskela         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_object		create_mesh(char *file, char *name, int verbose)
{
	...
	out.buffer = grid_triangulation(out.buffer, out.shape, out.dimensions.x, out.tri_cnt);
	out.model_mtx = compose_model(out, verbose);
	out.buffer = process_buffer(out.buffer, out.buffer, out.model_mtx, out.tri_cnt);
	if (verbose > 0)
	{
		printf("Mesh created!\n\n");
